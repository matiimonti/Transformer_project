...
def plot_loss_curves(all_metrics: dict, out_path: str):
    ...
    fig.suptitle(
        "Chess Transformer — Attention Variant Comparison",
        fontsize=14,
        fontweight="bold",
    )
...
def print_summary_table(summary: list):
    ...
    print("=" * 80)
    print(
        f"{'Variant':<16} {'Params':>8} {'Val Loss':>10} "
        f"{'PPL':>8} {'Legality':>10} {'Tok/s':>10}"
    )
    print("-" * 80)
    for s in summary:
        print(
...
def benchmark_compile(
    ...
    Returns {"eager_tok_per_sec": ..., "compiled_tok_per_sec": ...,
    "speedup": ...}.
    """
    ...
    def measure(model):
        ...
        with torch.no_grad():
            for _ in range(5):
                model(batch)  # (logits, None, None) — discarded intentionally
        if device.type == "cuda":
            torch.cuda.synchronize()
        t0 = time.time()
        with torch.no_grad():
...
def run_benchmark(checkpoint_dir: str, config: dict):
    ...
    for variant in VARIANTS:
        ...
        if metrics:
            ...
            # Move legality from final checkpoint
            entry["move_legality"] = last.get("move_legality", 0)
        else:
            print(
                f"[WARNING] No metrics for '{variant}' — "
                f"run train.py --variant {variant} first"
            )
    ...
    # torch.compile() speedup measured once on the vanilla variant
    ...
    if compile_result:
        print(
            ...
            f"\npeedup: {compile_result['speedup']:.2f}×"
        )
    ...
    print("Summary JSON saved to plots/benchmark_summary.json")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Benchmark ChessTransformer attention variants"
    )
