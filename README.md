...
```
├── src/
│   ├── attention.py      # All 4 attention variants
│   ├── model.py          # ChessTransformer (attention-agnostic via factory)
│   └── pgn_data.py       # PGN parser, tokenizer, Dataset
├── train.py              # Training loop (LR schedule, checkpointing, eval)
├── benchmark.py          # Throughput measurement and plot generation
├── notebooks/
│   ├── train_notebook.ipynb   # Unit tests — data pipeline, attention, model
...
for variant in vanilla rope gqa sparse; do
    python train.py --variant $variant --max_steps 3000 --out_dir checkpoints/$variant
done
