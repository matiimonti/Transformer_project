...
from pgn_data import ChessTokenizer, load_data
...
def train(config: dict):
    ...
    # (standard 6N approximation: 2 forward + 4 backward)
    tokens_per_step = (
        config["batch_size"]
        * config["seq_len"]
    ...
    print(f"\nStarting training: {max_steps} steps," f"warmup {warmup_steps} steps")
    print("-" * 60)
    ...
    while step < max_steps:
        ...
        # over `accum_steps` micro-batches before doing
        # an optimizer step.
        ...
        if step % config["eval_interval"] == 0:
            ...
            print(
                ...
                f" Val PPL: {val_ppl:.2f} | Move Legality: {legality:.1%}"
            )
            print(f"{'='*60}\n")
            ...
            if val_loss < best_val_loss:
                ...
                if patience_counter >= patience:
                    print(f"Early stopping at step {step}" f"(patience={patience})")
                    break
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Early stopping patience (evals w/o improvement). 0 = disabled",
    )
    parser.add_argument(
